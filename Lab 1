...
/* Edited by: John Hung, Spring 2023 */
/*   Cleaned up some comments */
/* ELEC 3040/3050 - Lab 1, Program 1 */
/* Toggle LED1 while button pressed, with delay inserted */
/* Count toggles */
...
/*---------------------------------------------------*/
void PinSetup () {
	/* Configure PA11 as input pin to read push button */
...
int main(void) {
 ...
 /* Endless loop */
	while (1) { //Can also use: for(;;) {
		if (led1 == 0) { //LED off
			GPIOB->BSRR = 0x0010 << 16; //Reset PB4=0 to turn OFF LED (in BSRR upper half)
		}
		else { //LED on
			GPIOB->BSRR = 0x0010; //Set PB4=1 to turn ON LED (in BSRR lower half)
		}
